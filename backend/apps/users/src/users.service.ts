...
export class UsersService {
  ...
  async getOrder(email:string,id:string){
    try{
      ...
      const order = await this.orderModel.findOne({id:id, email:email}).exec();
       console.log(order)
     
       
        const loadedItems = await Promise.all(
          (order?.items ?? []).map(async (it) => {
            const currProduct = await this.ProductModel.findById(it.product).exec() as any;
 ...
 async getCart(email: string) {
  ...
  async getALLShoppingList(email: string){
    ...
    try{
      ...
      const lists = await this. shoppingListModel.find({ _id: { $in: shoppingListIds } }).exec();
       console.log(lists)
