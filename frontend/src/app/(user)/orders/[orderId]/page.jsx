    ...
    return (
        <div className="max-w-7xl mx-auto p-6 space-y-6">
            ...
            <div className="bg-white shadow-md rounded-lg p-6 space-y-4">
                ...
                <a href={order.trackingLink} className="text-blue-600 underline">Track your order</a>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white shadow-md rounded-lg p-6 space-y-4">
                    ...
                    <p className="text-lg font-semibold">Net Amount: ₹{totalAmount.netAmount}</p>
                ...
                <div className="bg-white shadow-md rounded-lg p-6 space-y-4">
                    ...
                    {items.map((item, index) => (
                        <div key={index} className="flex items-center space-x-4 py-4 border-b border-gray-200">
                            <img src={item.product.imageLinks[0]} alt={item.product.name} className="w-16 h-16 object-cover" />
                            <div className="flex-1">
                                <p className="text-lg font-semibold">{item.product.name}</p>
                                <p className="text-sm text-gray-500">{item.product.description}</p>
                                <p className="text-sm text-gray-500">Price: ₹{item.originalPrice}</p>
                                <p className="text-sm text-gray-500">Quantity: {item.quantity}</p>
                                <p className="text-sm text-gray-500">Final Price: ₹{item.finalPrice}</p>
                            </div>
                        </div>
                    ))}
                </div>
