...
export default function InventoryPage() {
  ...
  const getStockStatus = (item) => {
    ...
    if (item.stock <= minStockQuantity) {
  ...
  const getStockPercentage = (item) => {
    ...
    console.log(
      `stock percen: ${Math.min(Math.round((item.stock / target) * 100), 100)}`,
    );
  ...
  function getDate(item) {
    return item ? new Date(item).toLocaleDateString().toLowerCase() : "";
  }
  return (
    <div className="flex flex-col gap-6">
      ...
      {lowStockItems.length > 0 && (
        <Alert variant="warning">
          ...
          <AlertDescription>
            {getLowStockItemCount()} products are running low on stock and need
            to be restocked.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              Across {category.length} categories
            </p>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{getOutOfStockItemCount()}</div>
            <p className="text-xs text-muted-foreground">
              ...
              {products.map((item) => {
                ...
                return (
                  <TableRow key={item._id}>
                    ...
                    <TableCell>
                      {getDate(item.updatedAt || item.createdAt || "")}
                    </TableCell>
